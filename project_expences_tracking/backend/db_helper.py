...
def get_db_cursor(commit=False):
    ...
    try:
        yield cursor
        if commit:
            connection.commit()  # Commit changes only if commit=True
...
def fetch_expense_summary_month():
    logger.info(f"fetch_expense_summary_month called with month:)")
    with get_db_cursor() as cursor:
        cursor.execute(
...
if __name__ == "__main__":
    ...
    expense_by_month = fetch_expense_summary_month()
